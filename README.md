...
- StyleGAN:
  - Blurring using depth-wise convolution is very slow in backward pass. However, since FIR filtering backward is identical to its forward, overriding backward pass solves the speed issue (subclass `torch.autograd.Function`). Gradients with respect to the kernel is also not required. StyleGAN/StyleGAN2 official codebase uses this approach.
  - Most popular re-implementations like MMGeneration, rosinality, PyTorch-StudioGAN use NVIDIA's custom CUDA kernel (upfirdn2d, introduced in StyleGAN2). I haven't benchmarked it, but I don't think the custom CUDA kernel is faster than depthwise convolution implementation.
  - TODO: benchmark FIR forward and backward. It seems like not overriding backward pass is faster at smaller image sizes. Even at large image sizes, overriding backward pass is not much faster. Perhaps it is due to gradient penalty, which requires 2nd-order gradient. Try train with normal GAN loss?
