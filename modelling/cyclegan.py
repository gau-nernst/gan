...
class ResNetGenerator(nn.Module):
    def __init__(
        ...
        self.layers.append(conv_norm_act(in_channels, base_channels, conv7x7, norm, act))

        for _ in range(downsample):
            self.layers.append(conv_norm_act(base_channels, base_channels * 2, conv3x3, norm, act, stride=2))
            base_channels *= 2

        for _ in range(n_blocks):
            self.layers.append(ResNetBlock(base_channels, base_channels, dropout, norm, act))

        for _ in range(downsample):
            self.layers.append(conv_norm_act(base_channels, base_channels // 2, upconv3x3, norm, act))
            base_channels //= 2

        self.layers.append(nn.Sequential(conv7x7(base_channels, out_channels), nn.Tanh()))
